...
class XPATHInjection(ActivePlugin):
  ...
  def ReportXPATHInjection(self, req_trigger, res_trigger):
    ...
    pr.Summary = "XPATH Injection has been detected in the '{0}' parameter of the {1} section of the request.<i<br>><i<br>>{2}<i<br>><i<br>>{3}".format(self.scnr.InjectedParameter, self.scnr.InjectedSection, self.GetSummary(), self.GetTrace())
  ...
  def GetReason(self, payload, errors):
    ...
    Reason = Reason + "IronWASP sent <i<hlg>>{0}<i</hlg>> as payload to the application, this payload would cause an exception to happen in insecure XPATH queries. ".format(payload)
    if len(errors) > 1:
    ...
    Reason = Reason + "Manually analyze the response received for the payload and confirm if the error message is actually because of some exception on the server-side. Resend the same payload but without the single or double quote and check if the error message disappears."
